...
def download_gdsc(
    ...
    """Download the pinned GDSC 8.4 source files.

    Existing files are reused, so repeated calls do not redownload
    the GDSC release.
    """
...
def _response_files(
    ...
    """Return the cached GDSC1/GDSC2 response files."""
    return sorted(
        Path(data_dir).glob(
            "GDSC[12]_fitted_dose_response_*.csv"
        )
    )


def _load_cell_line_metadata(
    metadata_path: Path,
) -> pd.DataFrame:
    """Load and standardize the GDSC cell-line metadata."""
...
def load_gdsc(
    ...
    metadata_path = (
        Path(data_dir) / SOURCE_FILES["cell_lines"]
    )
...
def filter_by_tissue(
    ...
    """Return rows matching a tissue without changing the source data."""
...
def validate_gdsc(
    ...
    """Validate GDSC source files and required structural fields."""
    ...
    if missing_files:
        raise FileNotFoundError(
            "Missing GDSC release files in "
            f"{data_dir!r}: {missing_files}"
        )
    ...
    for key in (
        ...
        missing = (
            RESPONSE_REQUIRED_COLUMNS - columns
        )
    ...
    if missing:
        raise ValueError(
            f"GDSC metadata is missing columns: "
            f"{sorted(missing)}"
        )
